function mapComponent(_hyvaData) {
    ...
    return {
        ...
        updateBranchData(deliveryBranchData) {
            ...
            this.selectedBranchPhone = deliveryBranchData.selected_branch_phone || null;
            ...
            window.initMapGlobal = () => {
                ...
                if (instance) {
                    ...
                    instance.initMap();
                     ...
                     console.error("[MapComponent] mapComponentInstance not found in Alpine store for initMapGlobal callback.");
                }
            };
        ...
        initMap() {
            if (typeof google === "undefined" || typeof google.maps === "undefined") {
                console.error("Google Maps API not loaded or 'google.maps' is undefined.");
                return;
            }
            const mapContainer = this.$refs.mapContainer || document.getElementById("amCountrySelector"); // Use x-ref if possible
            if (!mapContainer) {
                console.error("Map container (amCountrySelector or x-ref='mapContainer') not found.");
                return;
            }
        ...
        getValidCoordinates() {
            if (this.lastValidCoordinates && !isNaN(this.lastValidCoordinates.lat) && !isNaN(this.lastValidCoordinates.lng)) {
        ...
        createMarker(position) {
            if (!this.map || typeof google === "undefined" || !google.maps.Marker) {
                console.error("[MapComponent] Map or google.maps.Marker not available for createMarker.");
                return;
            }
            if (this.marker) {
        ...
        initAutocomplete() {
            if (!this.map || typeof google === "undefined" || !google.maps.places || !google.maps.places.Autocomplete) {
                console.error("[MapComponent] Autocomplete prerequisites not met (map or google.maps.places.Autocomplete).");
                return;
            }
            const input = this.$refs.addressSearch || document.getElementById("am-address-search"); // Use x-ref
            if (!input) {
                console.error("Address search input (am-address-search or x-ref='addressSearch') not found.");
                return;
            }
            ...
            this.autocomplete.addListener("place_changed", () => {
                ...
                if (!place.geometry || !place.geometry.location) {
                    this.showAddressError("Please select a valid address from the dropdown.");
                ...
                if(this.marker) this.marker.setPosition(location); else this.createMarker(location);
            ...
            input.addEventListener("blur", () => this.validateManualInput(input.value));
        ...
        validateManualInput(inputValue) {
            ...
            if (currentInputValue === "" && this.currentAddress === "") { // Both empty, nothing to validate
                 this.clearAddressError();
                 this.isAddressValid = false; // No address entered
                 return;
            }
            if (currentInputValue !== this.currentAddress) {
                this.showAddressError("Please select an address from the dropdown suggestions.");
                this.isAddressValid = false;
            } else if (currentInputValue === this.currentAddress && this.currentAddress !== "") {
                this.clearAddressError();
        ...
        showAddressError(message) {
            const input = this.$refs.addressSearch || document.getElementById("am-address-search");
        ...
        parseAddressComponents(components) {
            const componentMap = { street_number: "", route: "", sublocality: "", locality: "", administrative_area_level_1: "", postal_code: "", country: "" };
            components.forEach(component => {
                const type = component.types[0];
                if (componentMap.hasOwnProperty(type)) componentMap[type] = component.long_name;
                if (type === "country") componentMap.country = component.short_name; // Ensure country uses short_name
            });
            this.street = [componentMap.street_number, componentMap.route, componentMap.sublocality].filter(Boolean).join(", ");
        ...
        useCurrentLocation() {
            ...
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    if (this.map) { // Check if map is initialized
                        const newCenter = { lat, lng };
                        this.map.setCenter(newCenter);
                        if (this.marker) this.marker.setPosition(newCenter); else this.createMarker(newCenter);
                        this.reverseGeocodeAddress();
                    }
        ...
        async confirmLocation() {
            ...
            if (!this.isAddressValid) { // Check validation status
                 if (!document.getElementById("address-error-message")) { // Show generic error if specific one isn't there
            ...
            try {
                const nearestBranch = this.findNearestBranch(this.latitude, this.longitude, this._sourcesData);
                if (!nearestBranch) {
                    alert("Unable to find a branch within delivery range for this location.");
                ...
                await this.updateDeliveryBranchData();
        ...
        findNearestBranch(lat, lng, sourcesData) {
            ...
            sourcesData.forEach(source => {
                if (!source.latitude || !source.longitude || !source.delivery_range_km) return;
                const distance = this.calculateDistance(lat, lng, parseFloat(source.latitude), parseFloat(source.longitude));
        ...
        async saveAddressToServer() {
            if (!this.street || !this.city || !this.country ) { // Added country check
                console.error("Address information incomplete for saving:", {street: this.street, city: this.city, country: this.country });
            ...
            const addressData = {
                ...
                is_default_shipping: this.savedAddresses.length === 0 || (this.selectedAddress ? this.selectedAddress.is_default_shipping : true), // Smart default
                is_default_billing: this.savedAddresses.length === 0 || (this.selectedAddress ? this.selectedAddress.is_default_billing : false),
            };
            try {
        ...
        selectAddress(address) {
            if (address && typeof address.latitude !== 'undefined' && typeof address.longitude !== 'undefined') {
                this.selectedAddress = address;
                this.updateCoordinates(parseFloat(address.latitude), parseFloat(address.longitude));
                this.currentAddress = address.details || `${address.street}, ${address.city}`; // Fallback for details
                ...
                const nearestBranch = this.findNearestBranch(this.latitude, this.longitude, this._sourcesData);
                if (!nearestBranch) {
                    alert("Unable to find a branch within delivery range for the selected address.");
                ...
                this.updateDeliveryBranchData()
                    ...
                    .catch(error => console.error("Error updating branch data on selectAddress:", error))
        ...
        editAddress(address) {
            if (address && typeof address.latitude !== 'undefined' && typeof address.longitude !== 'undefined') {
                ...
                this.updateCoordinates(parseFloat(address.latitude), parseFloat(address.longitude));
                this.currentAddress = address.details || `${address.street}, ${address.city}`; // Populate search/display
                ...
                if (!this.googleMapsApiLoaded && this._apiKey) {
                    ...
                    setTimeout(() => {
                        ...
                        else { // Map exists, just update center and marker
                           const position = { lat: this.latitude, lng: this.longitude };
                           this.map.setCenter(position);
                           if(this.marker) this.marker.setPosition(position); else this.createMarker(position);
                        }
                 ...
                 alert("Selected address has invalid or missing coordinates for editing.");
            }
        },
    };
}
